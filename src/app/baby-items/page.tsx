...
export default async function Page() {
  ...
  const [items, pregnancy] = await Promise.all([
    itemsRepository.getItemsBySection(household.id, SECTIONS.babyItems.id),
  ...
  return (
    <div className="flex flex-col gap-6">
      <h1 className="text-3xl font-heading italic">{SECTIONS.babyItems.title_uk}</h1>
      <SectionChecklist sectionId={SECTIONS.babyItems.id} initialItems={items} currentWeek={currentWeek} />
    </div>
  )
}
