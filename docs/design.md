...
**Рядок "Пора зробити"** (дашборд): назва розділу (`text-xs
text-muted-foreground`) над назвою пункту (`text-sm`), далі `Badge
variant="secondary"` з дедлайном ("до N тижня") і чекбокс праворуч.
`secondary` — бо це нейтральна інформація про строк, не привід для
тривоги (destructive/яскраві акценти тут навмисно не використовуються).
Понад 8 пунктів — рідний `<details>/<summary>` ("Показати всі (N)"),
без нового client-компонента чи маршруту. Приклад:
`src/components/dashboard/due-checklist.tsx`.
